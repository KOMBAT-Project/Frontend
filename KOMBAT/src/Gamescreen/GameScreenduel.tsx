...
const GameScreen: React.FC<GameScreenProps> = ({
  ...
  useEffect(() => {
    ...
    // ✅ รับผลจาก backend หลัง END_TURN
  ...
  const handleHexClick = (q: number, r: number) => {
    ...
    if (phase === "SETUP_P1" || phase === "SETUP_P2") {
      ...
      sendAction({ playerIndex: isP1 ? 0 : 1, actionType: "SPAWN", row: r + 1, col: q + 1, unitId: selectedShopUnit });
      setSelectedShopUnit(null);
      ...
      sendAction({ playerIndex: isP1 ? 0 : 1, actionType: "PURCHASE_HEX", row: r + 1, col: q + 1 });
      setActionCompleted(true);
      setTimeout(() => handleNextPhase(true), 300);
  ...
  // ── Bot Turn (สำหรับ mode solitaire/auto) ────────────
  ...
  const handleNextPhase = (isAutoAdvance = false) => {
    ...
    switch (phase) {
      ...
      case "SETUP_P2":    setCurrentTurn(1); setPhase("P1_BUY_HEX"); break;
  ...
  useEffect(() => {
    ...
    if (isBot(phase) && !isExecuting && !waitingForBackend) {
      ...
      if (isBuyPhase || isSpawnPhase) {
        setTimeout(() => handleNextPhase(true), 300);
      } else if (isActionPhase) {
        simulateBotTurn(playerIdx);
      }
    }
  ...
  const getPhaseInstruction = () => {
    ...
    switch (phase) {
      ...
      case "SETUP_P2":    return "P2 SETUP: PLACE 1 STARTING MINION (FREE)";
      ...
      {phase === "GAME_OVER" && gameOverData && (
        <div className="game-over-overlay">
          <div className={`game-over-modal ${
            ...
            : gameOverData.winner.includes("02") ? "win-p2"
            : "draw"
          }`}>
